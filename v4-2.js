/* VYNTRO V4.2 — stable onboarding, editable profile, role-aware analytics */
(() => {
  ...
  let onboardingInitializedFor=null;
  let profileExtrasDirty=false;
  let goalDirty=false;
  let bound=false;

  function addAuthExtras(){
    const card=q('.auth-card'); if(!card) return;
    if(!q('#forgotPassword')){
  ...
  function ensureModals(){
    if(!q('#onboardingModal'))document.body.insertAdjacentHTML('beforeend',`
      <div id="onboardingModal" class="v41-modal hidden" aria-hidden="true"><div class="v41-backdrop"></div><div class="v41-modal-card">
        <div class="eyebrow">ПЕРВЫЕ 60 СЕКУНД</div><h2>Настроим твой путь</h2><p class="muted">Все поля сохраняются только после твоего действия и больше не сбрасываются автоматически.</p>
        <label class="field"><span>Как тебя называть</span><input id="onbName" maxlength="24" placeholder="Ник или имя" autocomplete="nickname"></label>
    ...
    q('#onbSave').onclick=()=>saveOnboarding(false);
    q('#onbSteam').onclick=()=>saveOnboarding(true);
    q('#saveNewPassword').onclick=saveNewPassword;
  }

  function showModal(id,on=true){
    const e=q(id);if(!e)return;
    e.classList.toggle('hidden',!on);e.setAttribute('aria-hidden',String(!on));
  ...
  function populateOnboarding(){
    if(!user)return;
    const m=meta();
    q('#onbName').value=(m.display_name&&m.display_name!=='Steam Player')?m.display_name:'';
    q('#onbRole').value=m.role||'Carry';
    q('#onbRank').value=m.target_rank||'Legend';
    q('#onbGames').value=m.games_per_week||'4-10';
    q('#onbProblem').value=m.self_problem||'unknown';
    q('#onbSteam').classList.toggle('hidden',!!steam64());
    q('#onbStatus').textContent='';
    onboardingInitializedFor=user.id;
  }

  function maybeOnboard(){
    if(!user||meta().onboarding_complete||q('#app')?.classList.contains('hidden'))return;
    const modal=q('#onboardingModal');if(!modal)return;
    if(modal.classList.contains('hidden')){
      if(onboardingInitializedFor!==user.id)populateOnboarding();
      q('#onbSteam').classList.toggle('hidden',!!steam64());
      showModal('#onboardingModal',true);
    }
    // ВАЖНО: если окно уже открыто, поля не трогаем. Пользователь может спокойно печатать.
  }

  async function saveOnboarding(connect){
    const name=q('#onbName').value.trim().slice(0,24);
    if(name.length<2){q('#onbStatus').textContent='Введите ник или имя минимум из 2 символов.';q('#onbName').focus();return;}
    q('#onbStatus').textContent='Сохраняю…';
    try{
      await updateMeta({display_name:name,role:q('#onbRole').value,target_rank:q('#onbRank').value,games_per_week:q('#onbGames').value,self_problem:q('#onbProblem').value,onboarding_complete:true,onboarding_version:2});
      onboardingInitializedFor=null;profileExtrasDirty=false;goalDirty=false;
      showModal('#onboardingModal',false);renderProfile();if(matches.length){analyze();renderAll()}
      toast(`Готово, ${name} ✓`);if(connect&&!steam64())await steamLogin();
    }catch(e){q('#onbStatus').textContent=cleanError(e)}
  }

  async function saveNewPassword(){
    const p=q('#newPassword').value;if(p.length<6)return q('#recoveryStatus').textContent='Минимум 6 символов.';
    const {error}=await sb.auth.updateUser({password:p});if(error)return q('#recoveryStatus').textContent=cleanError(error);
    q('#recoveryStatus').textContent='Пароль обновлён ✓';setTimeout(()=>showModal('#recoveryModal',false),700);
  }
  ...
  function heroPool(){
    const map={};matches.forEach(m=>{const x=map[m.hero_id]||(map[m.hero_id]={id:m.hero_id,games:0,wins:0,gpm:0,k:0,d:0,a:0});x.games++;x.wins+=win(m)?1:0;x.gpm+=(+m.gold_per_min||0);x.k+=(+m.kills||0);x.d+=(+m.deaths||0);x.a+=(+m.assists||0)});
    return Object.values(map).map(x=>({...x,wr:x.wins/x.games*100,avgGpm:x.gpm/x.games,kda:(x.k+x.a)/Math.max(1,x.d)})).sort((a,b)=>(b.games*8+b.wr*.4)-(a.games*8+a.wr*.4));
  }
  ...
  function renderV42(){
    const m=meta(),name=m.display_name||'Игрок',role=m.role||'Carry';
    const goal=q('#v41Goal');if(goal)goal.innerHTML=`<span>${role}</span><b>${m.target_rank||'Legend'}</b><small>${m.games_per_week||'—'} ranked / нед.</small>`;
    const self=q('#v41SelfProblem');if(self)self.textContent=PROBLEM_LABELS[m.self_problem]||'Не задано';
    const hp=heroPool(),pool=q('#v41HeroPool');
    if(pool)pool.innerHTML=!hp.length?'<div class="empty">Нужны матчи</div>':hp.slice(0,5).map((h,i)=>`<div class="hero-opt ${i<3?'recommended':''}"><span>${i+1}</span><div><b>${hname(h.id)}</b><small>${h.games} игр • ${Math.round(h.wr)}% WR</small></div><em>${h.kda.toFixed(1)} KDA</em></div>`).join('');
    const rec=q('#v41PoolAdvice');if(rec)rec.innerHTML=hp.length?`<b>Ranked pool:</b> ${hp.slice(0,3).map(x=>hname(x.id)).join(' • ')}<small>${hp[0].games<3?' Предварительная рекомендация — пока мало матчей.':' Основано на частоте и результатах последних матчей.'}</small>`:'';
    const t=trendData(),week=q('#v41Weekly');
    ...
    const mom=q('#v41Momentum');if(mom){const ms=matches.slice(0,10).reverse();mom.innerHTML=ms.length?ms.map(x=>`<span class="${win(x)?'mw':'ml'}" title="${hname(x.hero_id)} • ${+x.gold_per_min||0} GPM">${win(x)?'W':'L'}</span>`).join(''):'<div class="empty">Нет матчей</div>'}
    const focus=q('#v41NextFocus');if(focus)focus.innerHTML=analysis?`<span>Следующий фокус</span><b>${analysis.mission.title}</b><small>${analysis.mission.why}</small>`:'<span>Следующий фокус</span><b>Нужны данные</b>';
    if(!profileExtrasDirty){const ni=q('#displayName'),gpw=q('#gamesPerWeek'),sp=q('#selfProblem');if(ni)ni.value=name==='Steam Player'?'':name;if(gpw)gpw.value=m.games_per_week||'4-10';if(sp)sp.value=m.self_problem||'unknown'}
  }

  const baseRenderAll=renderAll;
  renderAll=function(){baseRenderAll();renderV42()};
  const baseRenderProfile=renderProfile;
  renderProfile=function(){
    const savedGoal=goalDirty?{role:q('#role')?.value,rank:q('#targetRank')?.value}:null;
    baseRenderProfile();
    if(savedGoal){if(q('#role'))q('#role').value=savedGoal.role;if(q('#targetRank'))q('#targetRank').value=savedGoal.rank}
    renderV42();
  ...
  function bindProfileExtras(){
    ...
    ['displayName','gamesPerWeek','selfProblem'].forEach(id=>{const e=q('#'+id);if(e){e.addEventListener('input',()=>profileExtrasDirty=true);e.addEventListener('change',()=>profileExtrasDirty=true)}});
    ['role','targetRank'].forEach(id=>q('#'+id)?.addEventListener('change',()=>goalDirty=true));
    const goalBtn=q('#saveGoal');if(goalBtn&&goalBtn.onclick){const original=goalBtn.onclick;goalBtn.onclick=async ev=>{await original.call(goalBtn,ev);goalDirty=false;renderV42()}};
    q('#saveProfileV41')?.addEventListener('click',async()=>{
      const name=q('#displayName').value.trim().slice(0,24);if(name.length<2)return toast('Ник должен содержать минимум 2 символа');
      try{await updateMeta({display_name:name,games_per_week:q('#gamesPerWeek').value,self_problem:q('#selfProblem').value});profileExtrasDirty=false;renderProfile();toast('Профиль обновлён ✓')}catch(e){toast(cleanError(e))}
    });
    q('#copyWeekly')?.addEventListener('click',async()=>{if(!analysis)return toast('Сначала нужны матчи');const hp=heroPool().slice(0,3).map(x=>hname(x.id)).join(', ');const text=`VYNTRO • ${meta().display_name||'Игрок'}\n${meta().role||'Carry'} → ${meta().target_rank||'Legend'}\nVYNTRO Score: ${analysis.score}\nWR: ${Math.round(analysis.wr)}% • KDA: ${analysis.kda.toFixed(1)}\nФокус: ${analysis.mission.title}\nRanked pool: ${hp}`;try{await navigator.clipboard.writeText(text);toast('Отчёт скопирован')}catch{toast('Не удалось скопировать')}});
  }

  async function handleSteamReturn(){
    const u=new URL(location.href),linked=u.searchParams.get('steam_linked'),err=u.searchParams.get('steam_error');
    if(linked==='1'){toast('Steam успешно привязан ✓');await sb.auth.refreshSession();await refreshUser();u.searchParams.delete('steam_linked');history.replaceState({},'',u.pathname+u.search+u.hash)}
    if(err){toast('Ошибка Steam: '+err);u.searchParams.delete('steam_error');history.replaceState({},'',u.pathname+u.search+u.hash)}
  }

  function bootV42(){
    addAuthExtras();ensureModals();bindProfileExtras();
    const badge=q('.brand small');if(badge)badge.textContent='V4.2';
    handleSteamReturn();
    sb.auth.onAuthStateChange((event,sess)=>{
      if(event==='PASSWORD_RECOVERY')showModal('#recoveryModal',true);
      if(event==='SIGNED_OUT'){onboardingInitializedFor=null;profileExtrasDirty=false;goalDirty=false}
      if(event==='SIGNED_IN'||event==='USER_UPDATED')setTimeout(()=>{if(sess?.user)maybeOnboard();renderV42()},180);
    });
    // Никаких setInterval: V4.2 полностью событийная и не перезаписывает ввод пользователя.
    if(matches.length){analyze();renderAll()}else renderV42();
    setTimeout(maybeOnboard,350);
  }

  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',bootV42);else bootV42();
})();
